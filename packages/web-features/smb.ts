...
const SPEC_ALIASES: Record<string, RegExp[]> = {
...
function parseSpecs(list: string): RegExp[] {
...
function matchesAllowedSpec(specs: string[] | undefined, patterns: RegExp[]): boolean {
  if (!specs || specs.length === 0) return false;
  return specs.some((s) => patterns.some((p) => p.test(s)));
}

function baselineTier(fid: string): "widely" | "newly" | "limited" {
...
function compliantAlternatives(fid: string, allowPatterns: RegExp[], prefer: "widely" | "newly"): string[] {
  const f = (features as any)[fid];
  const alts: string[] = [];
  ...
  if (alts.length === 0 && Array.isArray(f?.group)) {
    const groups = new Set<string>(f.group);
  ...
  const score = (id: string) => (baselineTier(id) === prefer ? 0 : baselineTier(id) === "widely" ? 1 : baselineTier(id) === "newly" ? 2 : 3);
  alts.sort((a, b) => score(a) - score(b));
  return Array.from(new Set(alts)).slice(0, 3);
}

function format(fid: string, explain: boolean): string {
...
function main(): void {
  ...
  const modeArg = (args.find((a) => a.startsWith("--mode="))?.split("=")[1] ?? "allow") as Mode;
  const prefer = (args.find((a) => a.startsWith("--prefer="))?.split("=")[1] ?? "widely") as "widely" | "newly";
